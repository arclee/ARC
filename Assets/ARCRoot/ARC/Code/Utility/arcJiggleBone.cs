...
//	Vector3 targetPos = new Vector3();
	...
	//使用 x 軸當 look at.
	public bool UseXAxisLookAt = false;
	...
	void Awake(){
		...
		Vector3 targetPos = FakeTrans.position + FakeTrans.TransformDirection(new Vector3((boneAxis.x * targetDistance),(boneAxis.y * targetDistance),(boneAxis.z * targetDistance)));
	...
	void JiggleBoneUpdate()
	{
		...
		FakeTrans.rotation = new Quaternion();

		// Update forwardVector and upVector
		Vector3 forwardVector = FakeTrans.TransformDirection(new Vector3((boneAxis.x * targetDistance),(boneAxis.y * targetDistance),(boneAxis.z * targetDistance)));
		Vector3 upVector = FakeTrans.TransformDirection(new Vector3(0,1,0));
		...
		if (UseXAxisLookAt)
		{
			Matrix4x4 lax = arcUtility.LookAtFromX(FakeTrans.position, dynamicPos, Vector3.up);		
			FakeTrans.rotation = arcUtility.QuaternionFromMatrix(lax);
		}
		else
		{
			FakeTrans.LookAt(dynamicPos, upVector);
		}
		...
		if(SquashAndStretch){
			...
			// Set the bone scale
			FakeTrans.localScale = new Vector3(xStretch, yStretch, zStretch);
			transform.localScale = FakeTrans.localScale;
		}
